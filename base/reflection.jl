...
"""
function isdispatchtuple(@nospecialize(t))
    @_total_meta
    return isa(t, DataType) && (t.flags & 0x4) == 0x4
end
...
"""
function typeintersect end
@eval function typeintersect(@nospecialize(a), @nospecialize(b))
    ...
    return $(Expr(:foreigncall, QuoteNode(:jl_type_intersection),
        Any, Expr(:call, Expr(:core, :svec), :Any, :Any), 0,
        QuoteNode((:ccall, 0x0f)),
        :a, :b))
end

@eval function morespecific(@nospecialize(a), @nospecialize(b))
    # equivalent to: @assume_effects :total @ccall jl_type_intersection(a::Any, b::Any)::Any
    return $(Expr(:foreigncall, QuoteNode(:jl_type_morespecific),
        Cint, Expr(:call, Expr(:core, :svec), :Any, :Any), 0,
        QuoteNode((:ccall, 0x0f)),
        :a, :b)) ≠ 0
end
...
```
"""
fieldoffset(x::DataType, idx::Integer) = (@_total_or_throw_meta; ccall(:jl_get_field_offset, Csize_t, (Any, Cint), x, idx))
...
```
"""
fieldtypes(T::Type) = (@_total_or_throw_meta; ntupleany(i -> fieldtype(T, i), fieldcount(T)))
